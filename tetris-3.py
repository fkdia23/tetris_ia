...
class Tetris:
    ...
    def draw(self) -> None:
        ...
        # Dessiner la pièce courante si le jeu n'est pas en pause
        if not self.paused:
            for i, row in enumerate(self.current_piece.shape):
                for j, cell in enumerate(row):
                    if cell:
        ...
        controls_text = font.render('P: Pause', True, WHITE)
        ...
        self.screen.blit(controls_text, (GRID_WIDTH * BLOCK_SIZE + 20, 270))
        self.screen.blit(restart_text, (GRID_WIDTH * BLOCK_SIZE + 20, 310))
        
        # Afficher le message de pause
        if self.paused:
            pause_text = font.render('PAUSE', True, WHITE)
            self.screen.blit(pause_text, 
                           (GRID_WIDTH * BLOCK_SIZE // 2 - 40, 
